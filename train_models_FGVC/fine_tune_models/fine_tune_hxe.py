...
CKPT_DIR  = os.path.join(BASE_DIR, f"checkpoints/fgvc/fine_tune_hxe_{ALPHA_TAG}")
TB_DIR    = os.path.join(BASE_DIR, f"dinov2_runs_fgvc/fine_tune_hxe_{ALPHA_TAG}")
...
child_leaf_masks  = torch.load("fgvc_child_leaf_masks.pt").float().to(device)
parent_leaf_masks = torch.load("fgvc_parent_leaf_masks.pt").float().to(device)
N_LEAVES_check, E, _ = child_leaf_masks.shape
assert N_LEAVES_check == N_LEAVES, "Mask/leaf_index mismatch"

depth_matrix = torch.from_numpy(
    pd.read_csv("fgvc_depth_matrix.csv", index_col=0).values.astype(np.float32)
).to(device)                                                  # [N_LEAVES, E]

edge_weights = torch.exp(-args.alpha * depth_matrix)         # [N_LEAVES, E]
...
S_full = torch.from_numpy(
    pd.read_csv("fgvc_sibling_matrix.csv", index_col=0).values.astype(np.float32)
)
