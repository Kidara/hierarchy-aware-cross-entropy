...
CKPT_DIR  = os.path.join(BASE_DIR, f"checkpoints/fgvc/fine_tune_softlabels_{BETA_TAG}")
TB_DIR    = os.path.join(BASE_DIR, f"dinov2_runs_fgvc/fine_tune_softlabels_{BETA_TAG}")
...
soft_targets = build_soft_targets(D, args.beta)              # [N_LEAVES, N_LEAVES]
...
class SoftLabelsLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        log_probs = F.log_softmax(logits, dim=1)              # [B, N_LEAVES]
        t         = self.soft_targets[targets]                # [B, N_LEAVES]
