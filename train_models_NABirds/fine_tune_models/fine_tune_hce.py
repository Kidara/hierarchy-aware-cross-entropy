...
CKPT_DIR  = os.path.join(BASE_DIR, "checkpoints/NABirds/fine_tune_hce_0.7_smooth_adj")
TB_DIR    = os.path.join(BASE_DIR, "dinov2_runs_NABirds/fine_tune_hce_0.7_smooth_adj")
FEAT_DIM  = 1024
N         = 1011
BATCH     = 1024
EPOCHS    = 2000
...
#hce loss function
def hierarchical_cross_entropy_loss(logits, targets, alpha = 0.1):
    ...
    smoothed_labels = torch.full((batch_size, N), alpha / (N - 1), device=logits.device)
...
for epoch in range(EPOCHS):
    ...
    for feats, targets in train_loader:
        ...
        optimizer.zero_grad()
        logits = head(feats)
        loss   = hierarchical_cross_entropy_loss(logits, targets)
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        for feats, targets in test_loader:
            feats, targets = feats.to(device), targets.to(device)
            logits = head(feats)
            rl += hierarchical_cross_entropy_loss(logits, targets).item()
