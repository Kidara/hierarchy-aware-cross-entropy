...
CKPT_DIR  = os.path.join(BASE_DIR, "checkpoints/NABirds/fine_tune_sce_0.7_smooth_adj")
TB_DIR    = os.path.join(BASE_DIR, "dinov2_runs_NABirds/fine_tune_sce_0.7_smooth_adj")
FEAT_DIM  = 1024
N_LEAVES  = 555
BATCH     = 1024
EPOCHS    = 2000
...
sibling_full = pd.read_csv("sibling_matrix.csv", index_col=0)
sibling_full.index   = sibling_full.index.astype(int)
sibling_full.columns = sibling_full.columns.astype(int)

S_small = np.zeros((N_LEAVES, N_LEAVES), dtype=np.float32)
for si, ti in seq_to_taxonomy.items():
    for sj, tj in seq_to_taxonomy.items():
        S_small[si, sj] = sibling_full.loc[ti, tj]
...
def load(split):
    ...
    feats  = d["features"]
    labels = torch.tensor([taxonomy_to_seq[l.item()] for l in d["fine_labels"]])
    return feats, labels
