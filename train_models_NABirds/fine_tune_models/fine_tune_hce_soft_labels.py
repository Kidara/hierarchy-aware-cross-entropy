...
CKPT_DIR  = os.path.join(BASE_DIR, f"checkpoints/NABirds/fine_tune_0.7_{args.beta}_adj")
TB_DIR    = os.path.join(BASE_DIR, f"dinov2_runs_NABirds/fine_tune_0.7_{args.beta}_adj")
...
R = torch.from_numpy(pd.read_csv("hierarchy_matrix.csv",
                     index_col=0).values.astype(np.float32)).to(device)
T = torch.from_numpy(pd.read_csv("diluted_encoding.csv",
                     index_col=0).values.astype(np.float32)).to(device)
S = torch.from_numpy(pd.read_csv("sibling_matrix.csv",
                     index_col=0).values.astype(np.float32)).to(device)
...
leaf_tensor = torch.tensor(leaf_list, dtype=torch.long, device=device)  #[555] original node IDs
...
S = torch.from_numpy(pd.read_csv("sibling_matrix.csv", index_col=0).values.astype(np.float32)).to(device)
...
class SoftLabelsLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        return hce_soft_labels(t, targets)
        # return -(t * log_probs).sum(dim=1).mean()

def hce_soft_labels(logits, targets):
    cell_type_probs = torch.softmax(logits, dim=-1)          # [batch, 1011]
    ...
    soft_leaf_dist = soft_targets[leaf_idx] # [batch, 555] using leaf indices within larger hierarchical space

    #scatter back into 1011-space so we can propagate to ancestors
    smoothed_labels = torch.zeros(targets.size(0), N, device=device)
    smoothed_labels[:, leaf_tensor] = soft_leaf_dist

    y_node = torch.matmul(smoothed_labels, T.T) #propagate to ancestors with diluted encoding
