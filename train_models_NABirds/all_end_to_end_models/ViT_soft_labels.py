...
parser.add_argument("--beta", type=float, default=10, choices=[10.0, 30.0],
...
CKPT_DIR  = os.path.join(BASE_DIR, f"checkpoints/NA_Birds/ViT_softlabels_{BETA_TAG}")
TB_DIR    = os.path.join(BASE_DIR, f"resnet_NABirds/ViT_softlabels_{BETA_TAG}")
...
model = models.vit_b_16(weights=None, dropout=DROPOUT)
model.heads = nn.Linear(model.heads.head.in_features, N_LEAVES)
...
# checkpoint_path = os.path.join(BASE_DIR, f"checkpoints/NA_Birds/resnet50_softlabels_{BETA_TAG}", "checkpoint.pth") #check if checkpoint_path exists so we restore where training left off
...
for epoch in range(EPOCHS):
    print(f"\nEpoch {epoch+1}/{EPOCHS}  [alpha={args.beta}]")
