...
class Params:
    def __init__(self):
        self.batch_size = 128
        self.name = "resnet18_hce_0.2_smooth_adj"
        self.workers = 8
        self.lr = 0.1 * 5
        self.momentum = 0.9
...
writer = SummaryWriter(os.path.join(BASE_DIR, "resnet18_NABirds2", params.name))

best_top1 = -1.0
best_top5 = -1.0
best_val_loss = float("inf")
start_epoch = 0
checkpoint_path = os.path.join(BASE_DIR, "checkpoints/NA_Birds", params.name, "checkpoint.pth") #check if checkpoint_path exists so we restore where training left off
