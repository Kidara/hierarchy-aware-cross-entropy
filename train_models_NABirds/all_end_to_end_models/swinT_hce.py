...
#hyperparameters from https://arxiv.org/pdf/2103.14030

class Params:
    def __init__(self):
        self.batch_size = 128
        self.name = "swinT_hce_0.5_smooth_adj"
        self.workers = 8
        self.lr = 1e-3 * (128 / 1024) * 2
        self.momentum = 0.9
...
model.head = nn.Linear(model.head.in_features, params.num_classes)
...
Path(os.path.join(BASE_DIR, "checkpoints/NABirds", params.name)).mkdir(parents=True, exist_ok=True)
writer = SummaryWriter(os.path.join(BASE_DIR, "swinT_NABirds", params.name))
...
for epoch in range(start_epoch, params.epochs):
    ...
    ckpt_dir = os.path.join(BASE_DIR, "checkpoints/NABirds", params.name)
