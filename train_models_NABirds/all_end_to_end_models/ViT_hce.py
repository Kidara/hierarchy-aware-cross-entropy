...
class Params:
    def __init__(self):
        self.batch_size = 64
        self.name = "vit_hce_0.5_smooth_adj"
        self.workers = 8
        self.lr = 3e-3 *2        #ViT-* ImageNet recipe (Dosovitskiy et al.)
        self.weight_decay = 0.3   #ViT-* ImageNet recipe (Dosovitskiy et al.)
        self.dropout = 0.1        #ViT-* ImageNet recipe (Dosovitskiy et al.)
        self.num_classes = 1011
        self.label_smoothing = 0.1
        self.epochs = 300         #ViT-* ImageNet recipe (Dosovitskiy et al.)
...
# NABirds-specific normalization
...
def train(dataloader, model, loss_fn, optimizer, epoch, writer):
    ...
    for batch, (X, taxonomy_labels) in enumerate(dataloader):
        ...
        loss.backward()
        # Gradient clipping — standard for ViT training from scratch
...
writer = SummaryWriter(os.path.join(BASE_DIR, "vit_NABirds", params.name))

best_top1 = -1.0
best_top5 = -1.0
best_val_loss = float("inf")
start_epoch = 0
checkpoint_path = os.path.join(BASE_DIR, "checkpoints/NABirds", params.name, "checkpoint.pth") #check if checkpoint_path exists so we restore where training left off
