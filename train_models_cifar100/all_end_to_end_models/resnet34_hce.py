...
from data_preprocessing import hierarchy_matrix, diluted_encoding, fine_to_siblings, normal_encoding, new_encoding
...
class Params:
    def __init__(self):
        self.batch_size = 128
        self.name = "resnet34_hce_0.5_no_smooth"
        self.workers = 8
        self.lr = 0.1
        self.momentum = 0.9
...
def train(dataloader, model, loss_fn, encoding, optimizer, epoch, writer, approx_size=50000):
    model.train()
    start0 = time.time()
    start = time.time()
    running_loss = 0.0
    ...
    for batch, (X, y_fine, y_coarse) in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            for i in range(len(y)):
                ...
                if predicted_label >= 0 and predicted_label in fine_to_siblings[true_label]:
                    correct_sibling += 1
...
def test(dataloader, model, loss_fn, encoding, epoch, writer):
    model.eval()
    test_loss = 0
    correct = 0
    correct_top5 = 0
...
for epoch in range(start_epoch, num_epochs):
    ...
    train_metrics = train(train_loader, model, hierarchical_cross_entropy_loss, T, optimizer, epoch=epoch, writer=writer)
    scheduler.step()
    test_metrics = test(test_loader, model, hierarchical_cross_entropy_loss, T, epoch=epoch, writer=writer)
...
# checkpoint_path_hce = os.path.join(BASE_DIR, "checkpoints", "resnet34_hce_0.2_adj2", "checkpoint_best_accuracy.pth")
# checkpoint_path_sce = os.path.join(BASE_DIR, "checkpoints", "resnet34_sce", "checkpoint_best_top5accuracy.pth")

# model_hce = models.resnet34(weights=None)
...
# model_sce = models.resnet34(weights=None)
...
# plot_path_top1 = os.path.join(BASE_DIR, "resnet34_hce_vs_sce_top1_per_class.png")
# plot_path_top5 = os.path.join(BASE_DIR, "resnet34_hce_vs_sce_top5_per_class.png")
...
#     title="Resnet 34 Per-class TOP-1 accuracy: SCE (not adjusted) baseline vs. HCE (adjusted LR to equivalent SCE LR)",
#     sort_by="sce",
# )
...
#     title="Resnet 34 Per-class TOP-5 accuracy: SCE (not adjusted) baseline vs. HCE (adjusted LR to equivalent SCE LR)",
#     sort_by="sce",
# )
