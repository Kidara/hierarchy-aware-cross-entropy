...
class Params:
    def __init__(self):
        self.batch_size = 128
        self.name = "resnet50_hce_0.2_no_smooth"
        self.workers = 8
        self.lr = 0.1
        self.momentum = 0.9
...
for epoch in range(start_epoch, num_epochs):
    ...
    train_metrics = train(train_loader, model, hierarchical_cross_entropy_loss, optimizer, scaler, epoch=epoch, writer=writer)
    warmup_scheduler.step()
    test_metrics = test(test_loader, model, hierarchical_cross_entropy_loss, epoch=epoch, writer=writer)
...
# checkpoint_path_hce = os.path.join(BASE_DIR, "checkpoints", "resnet50_hce_0.2_adj", "checkpoint_best_accuracy.pth") #check if checkpoint_path exists so we restore where training left off
# checkpoint_path_sce = os.path.join(BASE_DIR, "checkpoints", "resnet50_sce", "checkpoint_best_accuracy.pth")
...
# model_hce = models.resnet50(weights=None)
...
# model_sce = models.resnet50(weights=None)
...
#     title="Resnet 50 Per-class TOP-1 accuracy: SCE (not adjusted) baseline vs. HCE (adjusted LR to equivalent SCE LR)",
#     sort_by="sce",
# )
...
#     title="Resnet 50 Per-class TOP-5 accuracy: SCE (not adjusted) baseline vs. HCE (adjusted LR to equivalent SCE LR)",
#     sort_by="sce",
# )
