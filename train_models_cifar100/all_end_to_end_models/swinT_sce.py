...
class Params:
    def __init__(self):
        self.batch_size    = 128
        self.name          = "swinT_sce_0.7_no_smooth_adj"
        self.workers       = 8
        self.lr            = 1e-3 * (128 / 1024) / (10 / 7)
        self.momentum      = 0.9
...
writer = SummaryWriter(os.path.join(BASE_DIR, "swinT_cifar100", params.name))
...
for epoch in range(start_epoch, num_epochs):
    ...
    train_metrics = train(train_loader, model, nn.CrossEntropyLoss(), optimizer, epoch=epoch, writer=writer)
    scheduler.step()
    val_metrics = test(test_loader, model, nn.CrossEntropyLoss(), epoch=epoch, writer=writer) 
